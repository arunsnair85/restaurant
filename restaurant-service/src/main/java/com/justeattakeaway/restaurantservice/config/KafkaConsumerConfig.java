...
class KafkaConsumerConfig {
    ...
    @Value("${com.justeat.kafka.bootstrap-servers}")
    ...
    public ConsumerFactory<String, RestaurantStatusDTO> userConsumerFactory() {
        Map<String, Object> props = new HashMap<>();
        props.put(ConsumerConfig.BOOTSTRAP_SERVERS_CONFIG, bootstrapServers);
        props.put(ConsumerConfig.GROUP_ID_CONFIG, "reflectoring-user");
        return new DefaultKafkaConsumerFactory<>(props, new StringDeserializer(), new JsonDeserializer<>(RestaurantStatusDTO.class));
    }

    @Bean
    public ConcurrentKafkaListenerContainerFactory<String, RestaurantStatusDTO> userKafkaListenerContainerFactory() {
        ConcurrentKafkaListenerContainerFactory<String, RestaurantStatusDTO> factory = new ConcurrentKafkaListenerContainerFactory<>();
